...
import { isScheduledDay } from '@/lib/store/repo';
...
function ProgressContent() {
  ...
  const returnedDays = statuses.filter((status) => status === 'worked' || status === 'recovered').length;
  const scheduledDays = days.filter((date) => date <= localDate() && isScheduledDay(project.covenant, date)).length;
  const continuitySummary = `Returned ${returnedDays} of ${scheduledDays} scheduled days in the last 21 days.`;
  ...
  return <main className="page progress-page">
    ...
    <section className="progress-continuity" aria-labelledby="pattern-title"><div><h2 id="pattern-title" className="eyebrow">The recent pattern</h2><p>{continuitySummary}</p></div><div className="progress-squares" aria-hidden="true">{days.map((date, index) => <span key={date} className={statuses[index] === 'open' ? '' : statuses[index]} style={{ animationDelay: `${index * 18}ms` }} />)}</div><p className="sr-only">{continuitySummary} Unscheduled days are rest days.</p><ul className="sr-only">{days.map((date, index) => <li key={date}>{formatLongDate(date)}: {statuses[index]}</li>)}</ul><div className="continuity-legend"><span><i className="worked" />worked</span><span><i className="recovered" />returned</span><span><i className="declined" />declined</span><span><i className="missed" />missed</span><span><i className="rest" />rest</span></div></section>
