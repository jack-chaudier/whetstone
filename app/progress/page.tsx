...
import { continuitySummary, isScheduledDay, type ContinuityStatus } from '@/lib/store/repo';
...
function ProgressContent() {
  ...
  const continuityCaption = continuitySummary(project, days.map((date, index) => ({ date, status: statuses[index] })), 'in the last 21 days');
  ...
  return <main className="page progress-page">
    ...
    <section className="progress-continuity" aria-labelledby="pattern-title"><div><h2 id="pattern-title" className="eyebrow">The recent pattern</h2><p>{continuityCaption}</p></div><div className="progress-squares" aria-hidden="true">{days.map((date, index) => <span key={date} className={statuses[index] === 'open' ? '' : statuses[index]} style={{ animationDelay: `${index * 18}ms` }} />)}</div><p className="sr-only">{continuityCaption} Unscheduled days are rest days.</p><ul className="sr-only">{days.map((date, index) => <li key={date}>{formatLongDate(date)}: {statuses[index]}</li>)}</ul><div className="continuity-legend"><span><i className="worked" />worked</span><span><i className="recovered" />returned</span><span><i className="declined" />declined</span><span><i className="missed" />missed</span><span><i className="rest" />rest</span></div></section>
