  ...
  return <main className="page covenant-page">
    ...
    <section className="coach-voice" aria-labelledby="coach-voice-title">
      ...
      <fieldset className="provider-list"><legend className="sr-only">Coach provider</legend>
        <ProviderOption id="scripted" checked={state.coachProvider === 'scripted'} onChange={setCoachProvider} label="Whetstone scripted" detail="offline, always available" />
    ...
    {confirmDelete && <div className="dialog-backdrop"><section ref={deleteDialogRef} tabIndex={-1} className="dialog enter" role="dialog" aria-modal="true" aria-labelledby="delete-title"><p className="eyebrow">This cannot be undone</p><h2 id="delete-title" className="display delete-title">Delete the project and every session?</h2><p className="delete-copy">Export first if any part of the work should remain. Whetstone has no remote copy.</p><div className="delete-actions"><button className="quiet" onClick={cancelDelete}>Keep the project</button><button className="button delete-button" onClick={() => { clear(); router.push('/onboarding'); }}>Delete everything</button></div></section></div>}
  </main>;
}
