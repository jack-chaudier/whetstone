...
function RecoveryCard({ invitation, reason, onReason, onBegin, resume }: { invitation: Invitation | null; reason: RecoveryReason | null; onReason: (reason: RecoveryReason) => void; onBegin: () => void; resume: boolean }) {
  const tasks: Record<RecoveryReason, string> = { time: 'Open the unfinished edge. Mark the smallest place you would re-enter.', confusion: 'Read the last reflection. Name one question about what is unclear.', dread: 'Open the work and mark one part that still has life. Change nothing.', life: 'Spend ten minutes finding the edge again. Catching up is not today’s work.' };
