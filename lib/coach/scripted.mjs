const invitationTemplates = {
  make: [
    (c, last, minutes) => ({ action: `Make one imperfect move at the unfinished edge — ${milestoneClause(c)}.`, stopCondition: 'Stop when something in the work surprises you.', continuity: last, scopeMinutes: minutes }),
    ...
    (c, last, minutes) => ({ action: `Make one concrete choice at the live edge, then leave the polish alone. The near horizon: ${milestoneClause(c)}.`, stopCondition: 'Stop after the first consequential decision.', continuity: last, scopeMinutes: minutes }),
    (c, last, minutes) => ({ action: `Return to the live edge and add one imperfect piece — ${milestoneClause(c)}.`, stopCondition: 'Stop when the next decision becomes visible.', continuity: last, scopeMinutes: minutes }),
  ],
  learn: [
...
export class ScriptedCoachProvider {
  async generateInvitation(project, ctx) {
    ...
    const recovery = {
      make: 'Open the unfinished edge. Mark the smallest specific move that would resume it. Change nothing yet.',
  ...
  async assist(project, session, ask, level) {
    ...
    if (level === 'question') return project.covenant.shape === 'make'
      ? 'What is the smallest consequential choice at the live edge?'
    ...
    const options = project.covenant.shape === 'make'
      ? ['strengthen the clearest contrast', 'leave one useful tension unresolved', 'make the next move less safe and more specific']
      : project.covenant.shape === 'learn'
